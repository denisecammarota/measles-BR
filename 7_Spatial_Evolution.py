...
data_not = data_not.groupby(['ANO_EPI','ID_MUNICIP','GRUPO_IDADE'])['CASO'].sum()
data_not = data_not.reset_index()

# And now we will convert age groups to columns 
data_not = data_not.pivot_table(values='CASO', index=['ANO_EPI','ID_MUNICIP'], columns='GRUPO_IDADE', aggfunc='first')
data_not = data_not.reset_index()
data_not = data_not.fillna(0)
data_not = data_not.drop(columns = data_not.keys()[-1])

# We add all cases as well
data_not['TOTAL'] = data_not.iloc[:,2:].sum(axis = 1)

# Saving dataframe
data_not.to_csv('./Data/analyzed/not_spatial_year_evo.csv') 


# Separating based on residence municipality
data_res = data_total.copy()
data_res = data_res.groupby(['ANO_EPI','ID_MN_RESI','GRUPO_IDADE'])['CASO'].sum()
data_res = data_res.reset_index()

# And now we will convert age groups to columns 
data_res = data_res.pivot_table(values='CASO', index=['ANO_EPI','ID_MN_RESI'], columns='GRUPO_IDADE', aggfunc='first')
data_res = data_res.reset_index()
data_res = data_res.fillna(0)
data_res = data_res.drop(columns = data_res.keys()[-1])

# We add all cases as well
data_res['TOTAL'] = data_res.iloc[:,2:].sum(axis = 1)

# Saving dataframe
data_res.to_csv('./Data/analyzed/res_spatial_year_evo.csv') 
